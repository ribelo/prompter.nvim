...
---@param order fun(x, y)|nil
M._serialize = function(x, indent, order)
  ...
  if type(x) == "table" then
    ...
    if vim.tbl_islist(x) then
      ...
      for _, v in ipairs(x) do
        local value_lines = M._serialize(v, 0)
        for _, line in ipairs(value_lines) do
          table.insert(lines, indent_str .. "  - " .. line)
        end
      end
    else -- tabela jest tabela asocjacyjna
      ...
      for _, k in ipairs(keys) do
        ...
        ---@diagnostic disable-next-line: no-unknown
        local key_str = k
        local value_lines = M._serialize(v, indent + 2)
        if #value_lines == 1 then
          table.insert(lines, indent_str .. key_str .. ": " .. value_lines[1])
        else
          table.insert(lines, indent_str .. key_str .. ":")
          for _, line in ipairs(value_lines) do
            table.insert(lines, indent_str .. line)
          end
        end
...
---@param x any
---@param order fun(x, y)|nil
M.serialize = function(x, order)
  return M._serialize(x, 0, order)
end
